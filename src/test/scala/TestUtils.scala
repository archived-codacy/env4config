...
abstract trait TestUtils {
  ...
  protected def checkSerializationCompat[T: ClassTag](expectedHex: String, o: T, changedOK: Boolean = false): Unit = {
    ...
    try {
      assertEquals("Can no longer deserialize the old format of " + o.getClass.getSimpleName + why, o, deserialized)
...
, 11]""",
  ...
  protected def parseConfig(s: String) = {
    val options = ConfigParseOptions.defaults().setOriginDescription("test string").setSyntax(ConfigSyntax.CONF);
  ...
  def userDrive: String =
    if (isWindows) sys.props.get("user.dir").fold("")(_.takeWhile(_ != File.separatorChar)) else ""
  ...
  val resourceDir = {
    ...
    if (!f.exists()) {
      ...
      throw new Exception(
        s"Tests must be run from the root project directory containing ${f.getPath()}, however the current directory is $here"
      )
    }
    f
  }
  ...
  sealed abstract class Problem(path: String, line: Int) {
    ...
    protected def assertMessage(p: ConfigException.ValidationProblem, re: String) {
      assertTrue("didn't get expected message for " + path + ": got '" + p.problem() + "'", p.problem().matches(re))
    }
  }
